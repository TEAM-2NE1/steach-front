...
const RankingsList: React.FC<RankingsProps> = ({ data }) => {
  ...
  const [animatedScores, setAnimatedScores] = useState<number[]>(
    data ? data.prev.map((item) => item.score) : []
  );
      ...
      <ul style={{ listStyle: "none", padding: 0 }}>
        {displayedList.map((item, index) => {
          ...
          const currentRank = data
            ? data.current.find((currentItem) => currentItem.name === item.name)
                ?.rank
            : 0;
          ...
          return (
            <li
              key={item.name}
              className={`mb-1 py-0 px-5 shadow-md rounded-lg border border-gray-200 ${
